...
const XmlToJson = () => {
  ...
  const convertXmlToJson = () => {
    try {
      ...
      const xmlContent = xmlInput.trim().startsWith('<?xml') ? xmlInput : '<?xml version="1.0" encoding="UTF-8"?>\n' + xmlInput;
      const result = xml2json(xmlContent, { 
        compact: true, 
        spaces: 2,
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        setXmlInput(e.target?.result as string);
        setJsonOutput('');
      };
    ...
    <ClientPageTransition>
      <div className="space-y-6">
        <div className="text-center space-y-2">
          <h2 className="text-2xl font-semibold">XML to JSON Converter</h2>
          <p className="text-gray-400">Convert XML data to JSON format</p>
        </div>
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-lg p-4">
            ...
            <ul className="text-sm text-gray-400 space-y-1">
              <li>• Paste your XML data or upload an XML file</li>
              ...
              <li>• Make sure your XML is well-formed</li>
            </ul>
          </div>
        </div>
      </div>
